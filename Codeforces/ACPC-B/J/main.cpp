...
void hayakel() {
    ...
    for(int i = 0 ;i < n ; i++)
        cin >> a[i];
    ll sum_bef, sum_after;
    ll x;
    for(int i =0; i < n; i++){
        x = m;
        if(i) {
            sum_bef = accumulate(a, a + i, 0);
            x+=sum_bef;
        }
        else {
            sum_bef = LONG_LONG_MIN;
        }
        if(i < n-1) {
            sum_after = accumulate(a+i+1, a + n, 0);
            x+=sum_after;
        }
        else {
            sum_after = LONG_LONG_MIN;
        }
        if(sum_bef > x || sum_after >  x )
            x = (sum_bef > sum_after)? sum_bef : sum_after;
        cout << x << " ";
    }
