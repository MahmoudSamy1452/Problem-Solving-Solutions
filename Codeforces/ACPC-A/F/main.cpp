...
void hayakel() {
    ...
    while (x-- && !found) {
        cin >> killer >> killed;
        kills[killer].pb(killed);
        if(kills[killer].size() == 5){
            for(int i = 0; i < 4; i++) {
                found = true;
                if (std::count(kills[killer].begin(), kills[killer].end(),kills[killer][i]) > 1) {
                    for(int j = 1; j < 5; j++){
                        kills[killer][j-1] = kills[killer][j];
                    }
                    kills[killer].pop_back();
                    found = false;
                    break;
                }
            }
