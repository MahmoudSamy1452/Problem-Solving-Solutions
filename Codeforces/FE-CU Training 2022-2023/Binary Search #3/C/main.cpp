...
void hayakel() {
    string hamburger;
    cin >> hamburger;
    ll nburger[3] = {std::count(hamburger.begin(), hamburger.end(),'B'),
    std::count(hamburger.begin(), hamburger.end(),'S'),
    std::count(hamburger.begin(), hamburger.end(),'C')};
    ...
    while(left <= right){
        ...
        ll temp = rs;
        loop(i, 0, 3){
            temp -= max((mid*nburger[i] - n[i])*r[i], (ll)0);
            if(temp < 0)
                break;
        }
        if(temp < 0){
